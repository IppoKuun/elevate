...
export default async function bootstrapOwner(){
    ...
    if (!session) throw new classError.AuthRequiredError("NON CONNECTE")
    const sessionEmail = session?.user.email
    const sessionID = session.user.id
    if (sessionEmail !== ownerEmail)
    throw new classError.ForbiddenError(`EMAIL DE VOTRE SESSION NE CORRESPONDS PAS A CELUI DE OWNER. SESSION = ${session.user.email} ENV = ${process.env.OWNER_EMAIL}`)
